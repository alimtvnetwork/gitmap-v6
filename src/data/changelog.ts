...
export const changelog: ChangelogEntry[] = [
  {
    ...
    items: [
      ...
      "Docs site UI: version chip and command alias badges now use `text-foreground` with `dark:text-background` and `dark:bg-primary/25`, keeping text black/neutral and readable against the green tinted background in both light and dark themes. Touched components: `src/pages/Index.tsx` (hero version chip + CTA buttons restyled to `font-heading` with lift/shadow hover), `src/components/docs/DocsLayout.tsx` (header chip + new Sun/Moon theme toggle, persisted via `src/lib/theme.ts` and a pre-paint script in `index.html`), `src/components/docs/CommandCard.tsx` and `src/components/docs/CommandPalette.tsx` (alias badges), and `src/pages/VersionHistory.tsx` / `src/pages/Install.tsx` / `src/pages/CloneNext.tsx` (page-header alias chips). A single global override in `src/index.css` (`.dark [class*=\"bg-primary/\"].text-primary { color: hsl(var(--background)); }`) patches all remaining `bg-primary/N + text-primary` chips across `Architecture.tsx`, `Release.tsx`, `Doctor.tsx`, `Profile.tsx`, `Import.tsx`, `DiffProfiles.tsx`, `Changelog.tsx`, `PostMortems.tsx`, `ScanCloneFlags.tsx`, `GenericCLI.tsx`, and `ProjectDetection.tsx` without per-file edits.",
    ],
  },
  {
    version: "v3.11.1",
